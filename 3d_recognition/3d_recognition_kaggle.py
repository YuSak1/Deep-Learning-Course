...
class Network:
    ...
    def construct(self):
        with self.session.graph.as_default():
            # Inputs
            self.voxels = tf.placeholder(tf.float32, [None, modelnet_dim, modelnet_dim, modelnet_dim, 1], name="voxels")
            ...
            summary_writer = tf.contrib.summary.create_file_writer('LogFile', flush_millis=10 * 1000)
            self.summaries = {}
...
if __name__ == "__main__":
    ...
    # Load the data
    train, dev = Dataset("modelnet{}-train.npz".format(modelnet_dim)).split(train_split)
    test = Dataset("modelnet{}-test.npz".format(modelnet_dim), shuffle_batches=False)

    # Construct the network
    network = Network(threads=threads)
    network.construct()

    # Train
    for i in range(epochs):
        while not train.epoch_finished():
            voxels, labels = train.next_batch(batch_size)
    ...
    with open("3d_recognition_test.txt", "w") as test_file:
        while not test.epoch_finished():
            voxels, _ = test.next_batch(batch_size)
