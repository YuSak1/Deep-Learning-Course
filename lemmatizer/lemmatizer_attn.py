...
class Network:
    ...
    def construct(self, args, source_chars, target_chars, bow, eow):
        with self.session.graph.as_default():
            ...
            source_var = tf.get_variable("source_var", shape=[source_chars, args.char_dim])

            # TODO: Embed the self.source_seqs using the source embeddings.
            source_embedded = tf.nn.embedding_lookup(params=source_var, ids=self.source_seqs)
            ...
            fwd = tf.nn.rnn_cell.GRUCell(args.rnn_dim)
            bwd = tf.nn.rnn_cell.GRUCell(args.rnn_dim)
            (output_fw, output_bw), (state_fw, state_bw) = tf.nn.bidirectional_dynamic_rnn(cell_fw=fwd, cell_bw=bwd, inputs=source_embedded,
                                                                                        sequence_length=self.source_seq_lens, dtype=tf.float32)
            source_encoded = tf.add(output_fw, output_bw)
            source_states =  tf.add(state_fw, state_bw)
            ...
            target_var = tf.get_variable("target_var", shape=[target_chars, args.char_dim])

            # TODO: Embed the target_seqs using the target embeddings.
            target_embedded = tf.nn.embedding_lookup(params=target_var, ids=target_seqs)

            # TODO: Generate a decoder GRU with dimension args.rnn_dim.
            decoder = tf.nn.rnn_cell.GRUCell(args.rnn_dim)
            ...
            def with_attention(inputs, states):
                ...
                mult = tf.multiply(source_encoded, weight_project_sm)
                final_attn = tf.reduce_sum(mult, axis=1)
            ...
            class DecoderTraining(tf.contrib.seq2seq.Decoder):
                ...
                def initialize(self, name=None):
                    ...
                    embedded_bow = tf.nn.embedding_lookup(params=target_var, ids=tf.fill([self.batch_size], bow))
                    inputs = with_attention(inputs=embedded_bow, states=states)
                ...
                def step(self, time, inputs, states, name=None):
                    ...
                    next_input = with_attention(inputs=target_embedded[:,time], states=states)
            ...
            class DecoderPrediction(tf.contrib.seq2seq.Decoder):
                ...
                def initialize(self, name=None):
                    ...
                    # TODO: Call with_attention on the embedded BOW characters of shape [self.batch_size].
                    # You can use tf.fill to generate BOWs of appropriate size.
                    embedded_bow = tf.nn.embedding_lookup(target_var, tf.fill([self.batch_size], bow))
                    inputs = with_attention(inputs=embedded_bow, states=states)
                ...
                def step(self, time, inputs, states, name=None):
                    ...
                    next_input = with_attention(tf.nn.embedding_lookup(target_var, outputs), states)
