...
class Network:
    ...
    def construct(self, args, num_words, num_tags):
        with self.session.graph.as_default():
            ...
            we = tf.get_variable("var", shape=[num_words, args.we_dim])

            # TODO(we): Embed self.word_ids according to the word embeddings, by utilizing
            # `tf.nn.embedding_lookup`.
            embeded = tf.nn.embedding_lookup(params=we, ids=self.word_ids)
            ...
            log_likelihood, transition_params = tf.contrib.crf.crf_log_likelihood(inputs=output_layer, tag_indices=self.tags,
                                                                                sequence_lengths=self.sentence_lens)
            loss = tf.reduce_mean(-log_likelihood)

            # TODO: Compute the CRF predictions into `self.predictions` with `crf_decode`.
            self.predictions, _ = tf.contrib.crf.crf_decode(potentials=output_layer, transition_params=transition_params,
