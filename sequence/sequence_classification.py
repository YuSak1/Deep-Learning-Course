...
class Network:
    ...
    def construct(self, args):
        with self.session.graph.as_default():
            ...
            #initial_state = rnn_cell.zero_state(args.batch_size, dtype=tf.float32)
            ...
            hidden_layer, _ = tf.nn.dynamic_rnn(cell=rnn_cell, inputs=self.sequences, dtype=tf.float32)

            # TODO: If args.hidden_layer, add a dense layer with `args.hidden_layer` neurons
            # and ReLU activation.
            if args.hidden_layer is not None:
                hidden_layer = tf.layers.dense(hidden_layer, args.hidden_layer, tf.nn.relu)

            # TODO: Add a dense layer with one output neuron, without activation, into `output_layer`
            output_layer = tf.layers.dense(hidden_layer, 1, activation=None)
            
            # TODO: Remove the third dimension from `output_layer` using `tf.squeeze`.
            output_layer = tf.squeeze(output_layer, axis=2)
            ...
            gradient_norm = tf.global_norm(t_list=gradients)

            # TODO: If args.clip_gradient, clip gradients (back into `gradients`) using `tf.clip_by_global_norm`.
            if args.clip_gradient is not None:         
                gradients, gradient_norm = tf.clip_by_global_norm(t_list=gradients, clip_norm=args.clip_gradient, use_norm=gradient_norm)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sequence_dim", default=2, type=int, help="Sequence element dimension.")
