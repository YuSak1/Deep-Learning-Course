...
class Network:
    ...
    def construct(self, args):
        with self.session.graph.as_default():
            ...
            for i in range(self.TRAIN):
                ...
                if i == 0:
                    cell_out, state = rnn_cell(inputs=tf.zeros([1,1],tf.float32), state=state)
                else:
                    sequence = tf.reshape(self.sequence[i-1],[1,1])
                    cell_out, state = rnn_cell(inputs=sequence, state = state)                    

            
                # Then compute current prediction, by using `dense` layer, and append the scalar prediction
                # (i.e., with shape []) to `predictions`.
                output = tf.reshape(dense(inputs=cell_out),[])
            ...
            for i in range(self.TEST):
                ...
                prediction = tf.reshape(predictions[-1],[1,1])
                cell_out,state = rnn_cell(prediction, state)
                new_prediction = tf.reshape(dense(cell_out),[])
    ...
    def prediction_summary(self, gold, predictions):
        ...
        return self.session.run([self.summaries["prediction"],self.prediction_loss],
...
if __name__ == "__main__":
    ...
    for epoch in range(args.epochs):
        ...
        results = network.prediction_summary(data, network.predict(data[:Network.TRAIN]))

        # TODO: Print network.prediction_loss for each epoch, using "{:.2g}" format.
        print("{:.2g}".format(results[1]))