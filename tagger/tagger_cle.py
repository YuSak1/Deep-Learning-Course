...
class Network:
    ...
    def construct(self, args, num_words, num_chars, num_tags):
        with self.session.graph.as_default():
            ...
            we = tf.get_variable("var_we", shape=[num_words, args.we_dim])
            ...
            cle = tf.get_variable("var_cle", shape=[num_chars, args.cle_dim])

            # TODO: Embed self.charseqs (list of unique words in the batch) using the character embeddings.
            embeded_cle = tf.nn.embedding_lookup(params=cle, ids=self.charseqs)
            ...
            outputs_cle, (state_fw, state_bw) = tf.nn.bidirectional_dynamic_rnn(cell_fw=tf.nn.rnn_cell.GRUCell(args.cle_dim),
                                                    cell_bw=tf.nn.rnn_cell.GRUCell(args.cle_dim), inputs=embeded_cle,
            ...
            state_result =  tf.add(state_fw, state_bw)
            ...
            embeded_cle = tf.nn.embedding_lookup(params=state_result, ids=self.charseq_ids)

            # TODO: Concatenate the word embeddings (computed above) and the CLE (in this order).
            embeded =  tf.concat([embeded_we, embeded_cle], axis=2)
            ...
            outputs_we, _ = tf.nn.bidirectional_dynamic_rnn(cell_fw=fwd, cell_bw=bwd, inputs=embeded,
            ...
            outputs_we_concat =  tf.concat(outputs_we, axis=2)

            # TODO(we): Add a dense layer (without activation) into num_tags classes and
            # store result in `output_layer`.
            output_layer = tf.layers.dense(outputs_we_concat, num_tags, activation=None)
