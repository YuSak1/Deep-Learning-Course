...
class Network:
    ...
    def construct(self, args, num_words, num_chars, num_tags):
        with self.session.graph.as_default():
            ...
            # Training
            loss = tf.losses.sparse_softmax_cross_entropy(labels=self.languages, logits=output_layer, weights=weights)
            global_step = tf.train.create_global_step()
            self.training = tf.train.AdamOptimizer().minimize(loss, global_step=global_step, name="training")


            # Summaries
            self.current_accuracy, self.update_accuracy = tf.metrics.accuracy(self.languages, self.predictions, weights=weights)
        ...
        while not train.epoch_finished():
            ...
            self.session.run([self.training, self.summaries["train"]],
                             {self.sentence_lens: sentence_lens,
                              ...
                              self.languages: word_ids[train.languages]})
...
if __name__ == "__main__":
    ...
    network.construct(args, len(train.factors[train.FORMS].words), len(train.factors[train.FORMS].alphabet),
                      len(train.factors[train.languages].words))
    ...
    with open("{}/tagger_sota_test.txt".format(args.logdir), "w", encoding="utf-8") as test_file:
        ...
        for s in range(len(forms)):
            for i in range(len(forms[s])):
                print("{}\t_\t{}".format(forms[s][i], test.factors[test.languages].words[languages[s][i]]), file=test_file)
