    ...
    def construct(self, args):
        ...
        with self.session.graph.as_default():
            ...
            def encoder(image):
                ...
                dense1 = tf.layers.dense(flatten, 500, activation=tf.nn.relu)
                dense2 = tf.layers.dense(dense1, 500, activation=tf.nn.relu)

                z_mean = tf.layers.dense(dense2, self.z_dim, activation=None)
                z_log_variance = tf.layers.dense(dense2, self.z_dim, activation=None)
            ...
            def decoder(z):
                ...
                dense1 = tf.layers.dense(z, 500, activation=tf.nn.relu)
                dense2 = tf.layers.dense(dense1, 500, activation=tf.nn.relu)
                dense3 = tf.layers.dense(dense2, self.WIDTH * self.HEIGHT, activation=None)

                logits = tf.reshape(dense3, [-1, self.WIDTH, self.HEIGHT, 1])
                return logits
            ...
            self.loss = num_pixels * reconstruction_loss + self.z_dim * latent_loss
