...
class Network:
    ...
    def construct(self, args):
        ...
        with self.session.graph.as_default():
            ...
            def generator(z):
                ...
                dense2 = tf.layers.dense(dense1, self.WIDTH*self.HEIGHT, activation=tf.nn.sigmoid)

                logits = tf.reshape(dense2, [-1, self.WIDTH, self.HEIGHT, 1])
                return logits
            ...
            def discriminator(image):
                ...
                logit = tf.reshape(dense2, [args.batch_size, -1])
                return logit
            ...
            self.generator_training = tf.train.AdamOptimizer().minimize(self.generator_loss, var_list=tf.global_variables("generator"))
    ...
    def sample_z(self, batch_size):
        ...
        return np.random.uniform(low=-1, high=1, size=[batch_size, self.z_dim])
    ...
    def train(self, images):
        ...
        session1 = self.session.run([self.discriminator_training, self.discriminator_summary, self.discriminator_loss],
                                    {self.images: images, self.z: self.sample_z(args.batch_size)})
        ...
        session2 = self.session.run([self.generator_training, self.generator_summary, self.generator_loss],
                                    {self.z: self.sample_z(args.batch_size)})
