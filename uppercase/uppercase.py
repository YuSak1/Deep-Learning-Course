...
class Network:
    ...
    def construct(self, args, train):
        with self.session.graph.as_default():
            ...
            oh = tf.one_hot(self.windows, depth=args.alphabet_size)

            # TODO: Define a suitable network with appropriate loss function
            flattened_input = tf.layers.flatten(oh, name="input_layer")
            hidden_layer = tf.layers.dense(flattened_input, 300, activation=tf.nn.relu, name="hidden_layer")
            ...
            # decayed_lr=tf.train.exponential_decay(0.1, global_step, decay_steps, decay_rate, staircase=True)
            optimizer = tf.train.MomentumOptimizer(learning_rate=0.05, momentum=0.7).minimize(loss, global_step=global_step, name="training")
    ...
    def evaluate(self, dataset, windows, labels):
        return self.session.run([self.summaries[dataset], self.predictions, self.accuracy], {self.windows: windows, self.labels: labels})
...
if __name__ == "__main__":
    ...
    parser.add_argument("--alphabet_size", default=50, type=int, help="Alphabet size.")
    ...
    # Load the data
    train = Dataset("uppercase_data_train2.txt", args.window, alphabet=args.alphabet_size)
    ...
    for i in range(args.epochs):
        ...
        result = network.evaluate("dev", dev_windows, dev_labels)
        print("{:.2f}".format(result[2]*100))

    

    # TODO: Generate the uppercased test set
    test_windows, test_labels = test.all_data()
    result = network.evaluate("test", test_windows, test_labels)
    f = open('test_result.txt', "w", encoding='UTF-8')
    for i in range(len(test._text)):
        prediction = result[1]
        str = test._text[i]
        if prediction[i] == 1:
            str = str.upper() 
