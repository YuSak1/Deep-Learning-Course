...
class Network:
    ...
    def construct(self, args, train_size):
        with self.session.graph.as_default():
            ...
            lr_final = args.learning_rate_final
            ...
            if args.learning_rate_final != None:
                ...
                #print(lr_base*decay_rate**(args.epochs-1))
                decayed_lr=tf.train.exponential_decay(lr_base, global_step, decay_steps, decay_rate, staircase=True)
                
                if args.optimizer == "Adam":
                    optimizer = tf.train.AdamOptimizer(learning_rate=decayed_lr).minimize(loss, global_step=global_step, name="training")
                elif args.optimizer == "SGD":
                    if args.momentum == None:
                        optimizer = tf.train.GradientDescentOptimizer(learning_rate=decayed_lr).minimize(loss, global_step=global_step, name="training")
                    else:
                        optimizer = tf.train.MomentumOptimizer(learning_rate=decayed_lr, momentum=args.momentum).minimize(loss, global_step=global_step, name="training")
            else:
                if args.optimizer == "Adam":
                    optimizer = tf.train.AdamOptimizer(learning_rate=lr_base).minimize(loss, global_step=global_step, name="training")
                elif args.optimizer == "SGD":
                    if args.momentum == None:
                        optimizer = tf.train.GradientDescentOptimizer(learning_rate=lr_base).minimize(loss, global_step=global_step, name="training")
                    else:
                        optimizer = tf.train.MomentumOptimizer(learning_rate=lr_base, momentum=args.momentum).minimize(loss, global_step=global_step, name="training")
