    ...
    def construct(self, args):
        with self.session.graph.as_default():
            ...
            with tf.variable_scope("pretrain"):
                dense1 = tf.layers.dense(features, 700, activation=tf.nn.tanh)

                dense2 = tf.layers.dense(dense1, 200, activation=tf.nn.tanh)
                #dropout = tf.layers.dropout(dense2, rate=0.5, training=self.is_training)
                # dense3 = tf.layers.dense(dropout, 800, activation=tf.nn.relu)
                output_layer = tf.layers.dense(dense2, self.LABELS, activation=None)


            self.predictions = tf.argmax(output_layer, axis=1)

            training=tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,scope="pretrain")
            ...
            with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
                self.training = tf.train.AdamOptimizer().minimize(self.loss, var_list=training, global_step=global_step)
